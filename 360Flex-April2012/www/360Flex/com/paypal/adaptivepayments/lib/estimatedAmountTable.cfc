	...
	public any function deserialize(any jsonObj)
	{
		...
		if(isStruct(local.json))
		{
			...
			for(key in local.json)
			{
					...
					if('#key#' eq currName)
						if(NOT isStruct(local.json[key]) AND ('CurrencyConversionTable' eq "string" OR 'CurrencyConversionTable' eq "boolean" OR 'CurrencyConversionTable' eq "numeric"))
						{
							this.setestimatedAmountTable(local.json[key]);
						} else {
			...
			for( i = 1; i lte ArrayLen(local.json); i = i + 1)
			{
				...
				for(key in local.json[i])
				{
					...
					if('#key#' eq currName)
							if(NOT isStruct(local.json[i][key]) AND ('CurrencyConversionTable' eq "string" OR 'CurrencyConversionTable' eq "boolean" OR 'CurrencyConversionTable' eq "numeric"))
							{
								this.setestimatedAmountTable(local.json[i][key]);
							} else {
								var keyCom = 'estimatedAmountTable';
								if(keyCom eq 'ack')
									keyCom = 'ackCode';
